...
const REQUIRED_FORMAT_CSV =
  "question | type | subject | marks | negative_marks | difficulty | option_a | option_b | option_c | option_d | correct_answer | explanation";

export default function QuestionUploader({ source, onAdd, onCancel }) {
  ...
  const handleDownload = async () => {
    try {
      ...
      const res = await api.get(`/api/tests/question-templates/${cfg.ext}`, {
      ...
      a.href = url;
      a.download = `technical_questions_template.${cfg.ext}`;
  ...
  const validateFile = (file) => {
    ...
    if (!name.endsWith(cfg.ext)) {
      return `Invalid file type. Please upload a ${cfg.label} (.${cfg.ext}) file using the official Technical Questions template.`;
    }
              ...
              <tbody>
                {result.validQuestions.map((q, i) => (
                  <tr key={i} className="border-t admin-border">
                    ...
                    <td className="px-3 py-2" style={{ color: "var(--text-primary)" }}>{q.subject || "—"}</td>
