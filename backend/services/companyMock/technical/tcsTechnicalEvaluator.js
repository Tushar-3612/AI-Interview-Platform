import { isTcsAIConfigured, tcsAiGenerateJSON } from "../ai/tcsAiClient.js";
import { computeFallbackScore } from "./technicalFallback.js";

/**
 * TCS Company Mock — Dedicated Technical Evaluator.
 *
 * Evaluates free-text technical answers specifically for TCS Mock Interviews.
 * Uses tcsAiClient (TCS_MOCK_KEY) and strictly enforces score limits:
 * Easy = max 2, Medium = max 3, Hard = max 5.
 */

/**
 * Build the TCS technical evaluation prompt.
 */
export function buildTcsEvaluationPrompt({ question, topic, difficulty, candidateAnswer, expectedAnswer, explanation, betterAnswer, marks }) {
  const maxMarks = marks || (difficulty === "Easy" ? 2 : difficulty === "Hard" ? 5 : 3);
  const diffLabel = difficulty || "Medium";

  return `You are an expert technical interviewer evaluating a candidate's answer for a Software Engineer role at Tata Consultancy Services (TCS).
...
/**
 * Evaluate a candidate's technical answer for TCS.
 *
 ...
 */
export async function evaluateTcsSingleAnswer({
  question,
  topic,
  difficulty,
  candidateAnswer,
  ...
  // 2. Try TCS AI evaluation
  if (isTcsAIConfigured()) {
    try {
      const prompt = buildTcsEvaluationPrompt({
        question,
        topic,
        difficulty,
        candidateAnswer,
      ...
      const result = await tcsAiGenerateJSON(prompt, {
        temperature: 0.25,
        maxTokens: 1024,
      });
      ...
      console.log(`[TCS AI] Evaluation fallback triggered: ${error.message}`);
    }
  }
...
export const evaluateTcsTechnicalAnswer = (questionObj, candidateAnswer) => {
...
export function getTcsDeterministicFallback(questionObj, candidateAnswer) {
