/**
 * Comprehensive Test Suite for Benchmark IT Solutions Company Mock Implementation
 *
 * Verifies all 23 critical criteria:
 * 1. MCQ count >= 200
 ...
 * 16. Company-level isolation (Benchmark vs Celebal vs TCS vs Wipro vs Accenture)
 * 17. Resume behavior preservation
 * 18. Judge0 compatibility & testcase formatting
 * 19. Benchmark API isolation (reads ONLY BENCHMARK_MOCK_KEY / BENCHMARK_MOCK_MODEL)
 ...
 * 23. Central evaluation routing via evaluateSingleAnswer
 */
...
import { benchmarkConfig } from "../../config/companyMock/benchmark.js";
import { isBenchmarkAiConfigured, callBenchmarkAi } from "../../services/companyMock/ai/benchmarkAiClient.js";
import {
  evaluateBenchmarkSingleAnswer,
  getBenchmarkDeterministicFallback,
} from "../../services/companyMock/technical/benchmarkTechnicalEvaluator.js";
...
const benchmarkDataDir = path.resolve(rootDir, "data/companyMock/benchmark");
...
console.log("=== Starting Benchmark IT Solutions Company Mock Test Suite ===\n");
...
runTest("1. MCQ bank contains at least 200 questions", () => {
  const file = path.join(benchmarkDataDir, "mcq.json");
...
runTest("2. Technical bank contains at least 150 questions", () => {
  const file = path.join(benchmarkDataDir, "technical.json");
...
runTest("3. Coding bank contains at least 30 problems", () => {
  const file = path.join(benchmarkDataDir, "coding.json");
...
runTest("4. All question IDs are unique across all Benchmark banks", () => {
  const mcqs = JSON.parse(fs.readFileSync(path.join(benchmarkDataDir, "mcq.json"), "utf8"));
  const techs = JSON.parse(fs.readFileSync(path.join(benchmarkDataDir, "technical.json"), "utf8"));
  const codings = JSON.parse(fs.readFileSync(path.join(benchmarkDataDir, "coding.json"), "utf8"));
...
runTest("11. Random selection produces different question orders", () => {
  const loaded = loadCompanyMockTechnical("benchmark");
...
runTest("15. Student isolation ensures Student A exposure does not affect Student B", () => {
  const studentAExposures = new Set(["benchmark-tech-001", "benchmark-tech-002"]);
  const studentBExposures = new Set();

  assert.ok(!studentBExposures.has("benchmark-tech-001"), "Student B should not share Student A's exposures");
});

runTest("16. Company isolation ensures Benchmark questions never mix with Celebal, TCS, Wipro, or Accenture", () => {
  const benchmarkPool = loadCompanyQuestionsFromFolder("benchmark");
  for (const q of benchmarkPool) {
    assert.equal(q.companyId, "benchmark", `Found non-benchmark companyId in benchmark bank: ${q.companyId}`);
  }
});

runTest("17. Resume behavior preserves exact questions and ordering", () => {
  const mockAttempt = {
    selectedQuestions: ["benchmark-tech-010", "benchmark-tech-025", "benchmark-tech-050"],
    answers: [{ questionId: "benchmark-tech-010", answer: "My answer" }],
  };

  const resumedQuestions = [...mockAttempt.selectedQuestions];
  assert.deepEqual(resumedQuestions, ["benchmark-tech-010", "benchmark-tech-025", "benchmark-tech-050"]);
});

runTest("18. Judge0 compatibility verified with standardized language list and constraints", () => {
  const codings = loadCompanyMockCoding("benchmark");
  for (const q of codings) {
...
runTest("19. Benchmark API configuration strictly reads BENCHMARK_MOCK_KEY only", () => {
  const origBenKey = process.env.BENCHMARK_MOCK_KEY;
  ...
  try {
    process.env.BENCHMARK_MOCK_KEY = "benchmark_test_key_12345";
    ...
    assert.equal(benchmarkConfig.getApiKey(), "benchmark_test_key_12345");
    assert.equal(benchmarkConfig.isConfigured(), true);

    delete process.env.BENCHMARK_MOCK_KEY;
    assert.equal(benchmarkConfig.getApiKey(), "");
    assert.equal(benchmarkConfig.isConfigured(), false);
  } finally {
    if (origBenKey) process.env.BENCHMARK_MOCK_KEY = origBenKey; else delete process.env.BENCHMARK_MOCK_KEY;
...
await runAsyncTest("20. Missing BENCHMARK_MOCK_KEY triggers deterministic fallback with status='fallback'", async () => {
  const origBenKey = process.env.BENCHMARK_MOCK_KEY;
  try {
    delete process.env.BENCHMARK_MOCK_KEY;
    ...
    const res = await evaluateBenchmarkSingleAnswer({
      ...q,
    ...
    if (origBenKey) process.env.BENCHMARK_MOCK_KEY = origBenKey;
  }
});
...
await runAsyncTest("23. evaluateSingleAnswer routes companyId='benchmark' to Benchmark evaluator", async () => {
  const q = {
    companyId: "benchmark",
...
// ============================================================================
console.log("\n==================================================");
console.log(`Benchmark Test Suite Complete: ${passed} passed, ${failed} failed.`);
console.log("==================================================");
