...
/* ============================================================
   BRANDING / STATIC CONTENT (Aptitude)
   ============================================================ */

const PLATFORM_NAME = "AI-Powered Interview & Assessment Platform";
const DOC_TITLE = "Aptitude Question Import Template";
const PURPOSE =
  "Prepare your aptitude MCQs using this standardized template and upload the completed file through the Admin Panel. Keeping the provided structure unchanged ensures accurate question parsing and validation.";
const FORMAT_LINE =
  "This template shows you exactly how to structure your aptitude MCQs. Replace the placeholder text with your own questions. Do not rename or remove the field labels.";

const INSTRUCTIONS = [
  "Download the official template.",
  "Replace the placeholder text with your own questions.",
  "Keep all field names unchanged.",
  "Aptitude questions must use the MCQ format.",
  "Provide all four options: A, B, C, and D.",
  "Enter the correct answer using A, B, C, or D.",
  "Duplicate the complete question structure for additional questions.",
  "Save the completed file.",
  "Upload it through the Admin Panel → Aptitude Questions section.",
];

const QUESTION_LINES = [
  "Question ID:", "[Q001]", "",
  "Question:", "[Enter your aptitude question here]", "",
  "Type:", "[MCQ]", "",
  "Marks:", "[1]", "",
  "Negative Marks:", "[0]", "",
...
const LABEL_RE =
  /^(Question ID|Question|Type|Marks|Negative Marks|Difficulty|Options|Correct Answer|Explanation):\s*$/;

/* ============================================================
   CSV TEMPLATE
   ============================================================ */

export const TEMPLATE_CSV_TEXT = `question_id,question,type,marks,negative_marks,difficulty,option_a,option_b,option_c,option_d,correct_answer,explanation
[Q001],[Enter your aptitude question here],[MCQ],[1],[0],[Medium],[Enter option A],[Enter option B],[Enter option C],[Enter option D],[A],[Enter explanation]`;
...
/* ============================================================
   LOGO + MINIMAL DOCX (stored ZIP) HELPERS
   ============================================================ */
...
export function generateDocxTemplate() {
  ...
  bodyParts.push(docxPara("Aptitude Question Format", { bold: true, size: 28, color: "1F2937", spaceBefore: 200, spaceAfter: 60 }));
  bodyParts.push(docxPara(FORMAT_LINE, { size: 20, color: "6B7280", spaceAfter: 160 }));
  bodyParts.push(docxPara("APTITUDE QUESTION", { bold: true, size: 24, color: "1F2937", spaceBefore: 80, spaceAfter: 60 }));
  ...
  bodyParts.push(docxPara("Aptitude Question Import Template", { align: "center", size: 16, color: "9CA3AF", spaceAfter: 20 }));
...
<dc:title>Aptitude Question Import Template</dc:title>
...
export function generatePdfTemplate() {
  return new Promise((resolve, reject) => {
    try {
      ...
      drawRule(doc, left, right);
      doc.moveDown(0.4);
      doc.font("Helvetica-Bold").fontSize(12).fillColor(COLORS.heading).text("Aptitude Question Format", left, doc.y, { continued: false });
      doc.moveDown(0.25);
      doc.font("Helvetica").fontSize(9).fillColor(COLORS.sub).text(FORMAT_LINE, left, doc.y, { continued: false, width: contentWidth });
      doc.moveDown(0.6);

      doc.font("Helvetica-Bold").fontSize(11).fillColor(COLORS.heading).text("APTITUDE QUESTION", left, doc.y, { continued: false });
      doc.moveDown(0.3);
...
function drawFooter(doc) {
  ...
  doc.text("Use only the official template for reliable question import.", 55, page.height - 30, { align: "center", width: page.width - 110 });
  doc.restore();
}
