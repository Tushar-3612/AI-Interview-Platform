import { isAccentureAIConfigured, accentureAiGenerateJSON } from "../ai/accentureAiClient.js";
import { computeFallbackScore } from "./technicalFallback.js";

/**
 * Accenture Company Mock — Dedicated Technical Evaluator.
 *
 * Evaluates free-text technical / TITA answers specifically for Accenture Mock Interviews.
 * Uses accentureAiClient (ACCENTURE_MOCK_KEY) and strictly enforces score limits:
 * Easy = max 2, Medium = max 3, Hard = max 5.
 */

/**
 * Build the Accenture technical evaluation prompt.
 */
export function buildAccentureEvaluationPrompt({ question, topic, difficulty, candidateAnswer, expectedAnswer, explanation, betterAnswer, marks }) {
  const maxMarks = marks || (difficulty === "Easy" ? 2 : difficulty === "Hard" ? 5 : 3);
  const diffLabel = difficulty || "Medium";

  return `You are an expert technical interviewer evaluating a candidate's answer for an Associate Software Engineer / Software Engineer role at Accenture.

QUESTION: ${question}
TOPIC: ${topic || "Technical Fundamentals & TITA"}
...
EVALUATION CRITERIA:
1. Correctness & Technical Accuracy: Is the core technical concept or algorithm accurate?
2. Conceptual Understanding: Does the candidate grasp underlying principles, architecture, or mechanisms?
...
/**
 * Evaluate a candidate's technical answer for Accenture.
 *
 ...
 */
export async function evaluateAccentureSingleAnswer({
  question,
  topic,
  difficulty,
  candidateAnswer,
  ...
  // 2. Try Accenture AI evaluation
  if (isAccentureAIConfigured()) {
    try {
      const prompt = buildAccentureEvaluationPrompt({
        question,
        topic,
        difficulty,
        candidateAnswer,
      ...
      const result = await accentureAiGenerateJSON(prompt, {
        temperature: 0.25,
        maxTokens: 1024,
      });
      ...
      console.log(`[Accenture AI] Evaluation fallback triggered: ${error.message}`);
    }
  }
...
export const evaluateAccentureTechnicalAnswer = (questionObj, candidateAnswer) => {
...
export function getAccentureDeterministicFallback(questionObj, candidateAnswer) {
