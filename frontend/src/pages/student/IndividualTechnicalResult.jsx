...
export default function IndividualTechnicalResult() {
  ...
  const handleRetryEvaluation = async () => {
    setRetrying(true);
    const toastId = toast.loading("Retrying technical evaluation...");
    try {
      const headers = { Authorization: `Bearer ${token}` };
      const { data } = await api.post(`/api/individual/technical/result/${sessionId}/retry`, {}, { headers });
  ...
  const handleDownloadPDF = async () => {
    ...
    const toastId = toast.loading("Generating Technical Report PDF...");
    try {
      const headers = { Authorization: `Bearer ${token}` };
      const response = await api.get(`/api/individual/technical/result/${sessionId}/pdf`, {
        headers,
        responseType: "blob",
      });
      ...
      link.href = url;
      link.setAttribute("download", `Individual_Technical_Result_${sessionId}.pdf`);
      ...
      toast.error(err.response?.data?.message || "Unable to generate the PDF right now. Please try again.", { id: toastId });
    } finally {
      ...
      <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-center p-6 text-center space-y-4">
        ...
        <div className="flex gap-3">
          <button
            ...
            disabled={retrying}
            className="px-5 py-2.5 rounded-xl bg-orange-500 text-white font-bold text-xs uppercase cursor-pointer hover:bg-orange-600 disabled:opacity-50"
          >
