/**
 * Comprehensive Test Suite for Deloitte Company Mock Implementation
 *
 ...
 * 16. Company-level isolation (Deloitte vs Cognizant vs Capgemini vs Benchmark vs Accenture vs TCS vs Wipro vs Celebal)
 ...
 * 23. Routing of companyId='deloitte' through evaluateSingleAnswer to KEY2 evaluator
 */
...
const deloitteDataDir = path.resolve(rootDir, "data/companyMock/deloitte");
...
console.log("=== Starting Deloitte Company Mock Test Suite ===\n");
...
runTest("1. MCQ bank contains at least 200 questions", () => {
  const file = path.join(deloitteDataDir, "mcq.json");
...
runTest("2. Technical bank contains at least 150 questions", () => {
  const file = path.join(deloitteDataDir, "technical.json");
...
runTest("3. Coding bank contains at least 30 questions", () => {
  const file = path.join(deloitteDataDir, "coding.json");
...
runTest("4. Unique question IDs across all Deloitte banks", () => {
  const mcqs = JSON.parse(fs.readFileSync(path.join(deloitteDataDir, "mcq.json"), "utf8"));
  const techs = JSON.parse(fs.readFileSync(path.join(deloitteDataDir, "technical.json"), "utf8"));
  const codings = JSON.parse(fs.readFileSync(path.join(deloitteDataDir, "coding.json"), "utf8"));
...
runTest("11. Random selection produces different question orders", () => {
  const loaded = loadCompanyMockTechnical("deloitte");
...
runTest("15. Student isolation ensures Student A exposure does not affect Student B", () => {
  const studentAExposures = new Set(["deloitte-tech-1", "deloitte-tech-2"]);
  const studentBExposures = new Set();

  assert.ok(!studentBExposures.has("deloitte-tech-1"), "Student B should not share Student A's exposures");
});

runTest("16. Company isolation ensures Deloitte questions never mix with Cognizant, Capgemini, Benchmark, Accenture, TCS, Wipro, or Celebal", () => {
  const deloittePool = loadCompanyQuestionsFromFolder("deloitte");
  assert.ok(deloittePool.length >= 360, "Deloitte pool should have >= 360 combined questions");
  for (const q of deloittePool) {
    assert.equal(q.companyId, "deloitte", `Found non-deloitte companyId in deloitte bank: ${q.companyId}`);
  }
});
...
runTest("17. Resume behavior preserves exact questions and ordering", () => {
  const mockAttempt = {
    selectedQuestions: ["deloitte-tech-10", "deloitte-tech-25", "deloitte-tech-50"],
    answers: [{ questionId: "deloitte-tech-10", answer: "My answer" }],
  };

  const resumedQuestions = [...mockAttempt.selectedQuestions];
  assert.deepEqual(resumedQuestions, ["deloitte-tech-10", "deloitte-tech-25", "deloitte-tech-50"]);
});

runTest("18. Judge0 compatibility verified with standardized language list and constraints", () => {
  const codings = loadCompanyMockCoding("deloitte");
...
runTest("19. Deloitte API configuration strictly reads MOCK_INTERVIEW_API_KEY2 only", () => {
  ...
  try {
    process.env.MOCK_INTERVIEW_API_KEY2 = "deloitte_test_key_12345";
    ...
    assert.equal(SHARED2_CONFIG.getApiKey(), "deloitte_test_key_12345");
...
await runAsyncTest("23. evaluateSingleAnswer routes companyId='deloitte' to KEY2 evaluator", async () => {
  const q = {
    companyId: "deloitte",
...
// ============================================================================
console.log("\n==================================================");
console.log(`Deloitte Test Suite Complete: ${passed} passed, ${failed} failed.`);
console.log("==================================================");
