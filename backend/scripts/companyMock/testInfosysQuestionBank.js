/**
 * Comprehensive Test Suite for Infosys Company Mock Implementation
 *
 * Verifies all 24 critical criteria:
 * 1. MCQ count >= 200
 ...
 * 16. Company-level isolation (Infosys vs Deloitte vs Cognizant vs Capgemini vs Benchmark vs Accenture vs TCS vs Wipro vs Celebal)
 ...
 * 22. System Design question validation (>= 20 System Design / Architecture questions)
 * 23. Strict absence of project / resume questions
 * 24. Routing of companyId='infosys' through evaluateSingleAnswer to KEY2 evaluator
 */
...
const infosysDataDir = path.resolve(rootDir, "data/companyMock/infosys");
...
console.log("=== Starting Infosys Company Mock Test Suite ===\n");
...
runTest("1. MCQ bank contains at least 200 questions", () => {
  const file = path.join(infosysDataDir, "mcq.json");
...
runTest("2. Technical bank contains at least 150 questions", () => {
  const file = path.join(infosysDataDir, "technical.json");
...
runTest("3. Coding bank contains at least 30 questions", () => {
  const file = path.join(infosysDataDir, "coding.json");
...
runTest("4. Unique question IDs across all Infosys banks", () => {
  const mcqs = JSON.parse(fs.readFileSync(path.join(infosysDataDir, "mcq.json"), "utf8"));
  const techs = JSON.parse(fs.readFileSync(path.join(infosysDataDir, "technical.json"), "utf8"));
  const codings = JSON.parse(fs.readFileSync(path.join(infosysDataDir, "coding.json"), "utf8"));
...
runTest("11. Random selection produces different question orders", () => {
  const loaded = loadCompanyMockTechnical("infosys");
...
runTest("15. Student isolation ensures Student A exposure does not affect Student B", () => {
  const studentAExposures = new Set(["infosys-tech-1", "infosys-tech-2"]);
  const studentBExposures = new Set();

  assert.ok(!studentBExposures.has("infosys-tech-1"), "Student B should not share Student A's exposures");
});

runTest("16. Company isolation ensures Infosys questions never mix with Deloitte, Cognizant, Capgemini, Benchmark, Accenture, TCS, Wipro, or Celebal", () => {
  const infosysPool = loadCompanyQuestionsFromFolder("infosys");
  assert.ok(infosysPool.length >= 360, "Infosys pool should have >= 360 combined questions");
  for (const q of infosysPool) {
    assert.equal(q.companyId, "infosys", `Found non-infosys companyId in infosys bank: ${q.companyId}`);
  }
});
...
// ============================================================================
console.log("\n--- 3. System Design, Resume, Judge0 & KEY2 AI Evaluator Integration ---");

runTest("17. Resume behavior preserves exact questions and ordering", () => {
  const mockAttempt = {
    selectedQuestions: ["infosys-tech-10", "infosys-tech-25", "infosys-tech-50"],
    answers: [{ questionId: "infosys-tech-10", answer: "My answer" }],
  };

  const resumedQuestions = [...mockAttempt.selectedQuestions];
  assert.deepEqual(resumedQuestions, ["infosys-tech-10", "infosys-tech-25", "infosys-tech-50"]);
});

runTest("18. Judge0 compatibility verified with standardized language list and constraints", () => {
  const codings = loadCompanyMockCoding("infosys");
...
runTest("19. Infosys API configuration strictly reads MOCK_INTERVIEW_API_KEY2 only", () => {
  ...
  try {
    process.env.MOCK_INTERVIEW_API_KEY2 = "infosys_test_key_12345";
    ...
    assert.equal(SHARED2_CONFIG.getApiKey(), "infosys_test_key_12345");
...
await runAsyncTest("20. Missing MOCK_INTERVIEW_API_KEY2 triggers deterministic fallback with status='fallback'", async () => {
  ...
  try {
    ...
    const q = {
      companyId: "infosys",
      question: "Explain the Singleton Design Pattern and its potential drawbacks.",
      expectedAnswer: "Singleton ensures a class has only one instance and provides global access.",
      explanation: "Useful for connection pools but introduces global state.",
      betterAnswer: "Singleton restricts instantiation to one object per JVM.",
      difficulty: "Medium",
      marks: 3,
    };
    ...
    const res = await evaluateShared2SingleAnswer({
      ...q,
      candidateAnswer: "Singleton pattern ensures a class has only a single instance.",
    });
...
runTest("22. System Design questions presence and structural validation (>= 20 questions)", () => {
  const techs = JSON.parse(fs.readFileSync(path.join(infosysDataDir, "technical.json"), "utf8"));
  const mcqs = JSON.parse(fs.readFileSync(path.join(infosysDataDir, "mcq.json"), "utf8"));

  const sysDesignTechs = techs.filter((q) => q.topic === "System Design" || /design|architecture|solid|pattern|microservice/i.test(q.topic || ""));
  const sysDesignMcqs = mcqs.filter((q) => q.topic === "System Design" || /design|architecture|solid|pattern|microservice/i.test(q.topic || ""));

  assert.ok(sysDesignTechs.length >= 20, `Expected >= 20 System Design technical questions, found ${sysDesignTechs.length}`);
  assert.ok(sysDesignMcqs.length >= 20, `Expected >= 20 System Design MCQs, found ${sysDesignMcqs.length}`);
});

runTest("23. Strict absence of project / resume / personal candidate questions", () => {
  const techs = JSON.parse(fs.readFileSync(path.join(infosysDataDir, "technical.json"), "utf8"));
  const mcqs = JSON.parse(fs.readFileSync(path.join(infosysDataDir, "mcq.json"), "utf8"));
  const codings = JSON.parse(fs.readFileSync(path.join(infosysDataDir, "coding.json"), "utf8"));
...
await runAsyncTest("24. evaluateSingleAnswer routes companyId='infosys' to KEY2 evaluator", async () => {
  const q = {
    companyId: "infosys",
    question: "Design a simple URL shortener at a high level.",
    expectedAnswer: "Convert long URLs to short 6-char Base62 keys backed by Redis and NoSQL DB.",
    explanation: "Base62 encoding of unique auto-increment IDs provides compact keys.",
    betterAnswer: "High-level design includes API Gateway, Base62 Key Service, Redis Cache, and DB.",
    difficulty: "Hard",
    marks: 5,
  };
  ...
  const res = await evaluateSingleAnswer({
    ...q,
    candidateAnswer: "A URL shortener uses Base62 encoding on unique IDs and caches URLs in Redis.",
  });
...
// ============================================================================
console.log("\n==================================================");
console.log(`Infosys Test Suite Complete: ${passed} passed, ${failed} failed.`);
console.log("==================================================");
