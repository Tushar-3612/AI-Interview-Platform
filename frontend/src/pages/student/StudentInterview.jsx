...
function StudentInterview() {
  ...
  const handleUploadAndStart = async () => {
    ...
    setIsLoading(true);
    try {
      // 1. Upload and analyze resume (saves it to the student profile)
      await api.post("/api/student/resume/upload", formData, {
        headers: {
      ...
      // 2. Start the interview — AI CALL #1 generates 25 Technical + 5 HR + 3
      //    Coding (one request) + 25 Aptitude (local), returns all questions.
      ...
      const generatedQuestions = startRes.data.generatedQuestions || [];
      ...
      setInterviewId(startRes.data._id || startRes.data.interviewId);
      toast.success("AI Interview session initialized successfully!");
    } catch (error) {
