/**
 * Comprehensive Test Suite for Capgemini Company Mock Implementation
 *
 ...
 * 16. Company-level isolation (Capgemini vs Celebal vs TCS vs Wipro vs Accenture vs Benchmark)
...
const capgeminiDataDir = path.resolve(rootDir, "data/companyMock/capgemini");
...
console.log("=== Starting Capgemini Company Mock Test Suite ===\n");
...
runTest("1. MCQ bank contains at least 200 questions", () => {
  const file = path.join(capgeminiDataDir, "mcq.json");
...
runTest("2. Technical bank contains at least 150 questions", () => {
  const file = path.join(capgeminiDataDir, "technical.json");
...
runTest("3. Coding bank contains at least 30 problems", () => {
  const file = path.join(capgeminiDataDir, "coding.json");
...
runTest("4. All question IDs are unique across all Capgemini banks", () => {
  const mcqs = JSON.parse(fs.readFileSync(path.join(capgeminiDataDir, "mcq.json"), "utf8"));
  const techs = JSON.parse(fs.readFileSync(path.join(capgeminiDataDir, "technical.json"), "utf8"));
  const codings = JSON.parse(fs.readFileSync(path.join(capgeminiDataDir, "coding.json"), "utf8"));
...
runTest("11. Random selection produces different question orders", () => {
  const loaded = loadCompanyMockTechnical("capgemini");
...
runTest("15. Student isolation ensures Student A exposure does not affect Student B", () => {
  const studentAExposures = new Set(["capgemini-tech-001", "capgemini-tech-002"]);
  const studentBExposures = new Set();

  assert.ok(!studentBExposures.has("capgemini-tech-001"), "Student B should not share Student A's exposures");
});

runTest("16. Company isolation ensures Capgemini questions never mix with Celebal, TCS, Wipro, Accenture, or Benchmark", () => {
  const capgeminiPool = loadCompanyQuestionsFromFolder("capgemini");
  for (const q of capgeminiPool) {
    assert.equal(q.companyId, "capgemini", `Found non-capgemini companyId in capgemini bank: ${q.companyId}`);
  }
});

runTest("17. Resume behavior preserves exact questions and ordering", () => {
  const mockAttempt = {
    selectedQuestions: ["capgemini-tech-010", "capgemini-tech-025", "capgemini-tech-050"],
    answers: [{ questionId: "capgemini-tech-010", answer: "My answer" }],
  };

  const resumedQuestions = [...mockAttempt.selectedQuestions];
  assert.deepEqual(resumedQuestions, ["capgemini-tech-010", "capgemini-tech-025", "capgemini-tech-050"]);
});

runTest("18. Judge0 compatibility verified with standardized language list and constraints", () => {
  const codings = loadCompanyMockCoding("capgemini");
  for (const q of codings) {
...
runTest("19. Capgemini API configuration strictly reads MOCK_INTERVIEW_API_KEY2 only", () => {
  ...
  try {
    process.env.MOCK_INTERVIEW_API_KEY2 = "capgemini_test_key_12345";
    ...
    assert.equal(SHARED2_CONFIG.getApiKey(), "capgemini_test_key_12345");
...
await runAsyncTest("23. evaluateSingleAnswer routes companyId='capgemini' to KEY2 evaluator", async () => {
  const q = {
    companyId: "capgemini",
...
// ============================================================================
console.log("\n==================================================");
console.log(`Capgemini Test Suite Complete: ${passed} passed, ${failed} failed.`);
console.log("==================================================");
