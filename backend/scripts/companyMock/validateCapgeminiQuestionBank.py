#!/usr/bin/env python3
"""
Validation script for Capgemini Company Mock Question Bank.

Usage:
    python backend/scripts/companyMock/validateCapgeminiQuestionBank.py
"""
...
CAPGEMINI_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "../../data/companyMock/capgemini"))

MCQ_FILE = os.path.join(CAPGEMINI_DIR, "mcq.json")
TECH_FILE = os.path.join(CAPGEMINI_DIR, "technical.json")
CODING_FILE = os.path.join(CAPGEMINI_DIR, "coding.json")
...
def main():
    print("=== Validating Capgemini Company Mock Question Bank ===\n")
    errors = []
    warnings = []
    ...
    for i, q in enumerate(mcqs):
        ...
        if q.get("companyId") != "capgemini":
            errors.append(f"[MCQ: {qid}] companyId must be 'capgemini', got '{q.get('companyId')}'")

        cids = q.get("companyIds")
        if not isinstance(cids, list) or cids != ["capgemini"]:
            errors.append(f"[MCQ: {qid}] companyIds must be ['capgemini'], got {cids}")
    ...
    for i, q in enumerate(techs):
        ...
        if q.get("companyId") != "capgemini":
            errors.append(f"[Technical: {qid}] companyId must be 'capgemini', got '{q.get('companyId')}'")

        cids = q.get("companyIds")
        if not isinstance(cids, list) or cids != ["capgemini"]:
            errors.append(f"[Technical: {qid}] companyIds must be ['capgemini'], got {cids}")
    ...
    for i, q in enumerate(codings):
        ...
        if q.get("company") != "capgemini":
            errors.append(f"[Coding: {qid}] company must be 'capgemini', got '{q.get('company')}'")
