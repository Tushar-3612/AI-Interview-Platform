#!/usr/bin/env python3
"""
Validation script for Deloitte Company Mock Question Bank.

Usage:
    python backend/scripts/companyMock/validateDeloitteQuestionBank.py
"""
...
DELOITTE_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "../../data/companyMock/deloitte"))

MCQ_FILE = os.path.join(DELOITTE_DIR, "mcq.json")
TECH_FILE = os.path.join(DELOITTE_DIR, "technical.json")
CODING_FILE = os.path.join(DELOITTE_DIR, "coding.json")
...
def main():
    print("=== Validating Deloitte Company Mock Question Bank ===\n")
    errors = []
    warnings = []
    ...
    for i, q in enumerate(mcqs):
        ...
        if q.get("companyId") != "deloitte":
            errors.append(f"[MCQ: {qid}] companyId must be 'deloitte', got '{q.get('companyId')}'")

        cids = q.get("companyIds")
        if not isinstance(cids, list) or cids != ["deloitte"]:
            errors.append(f"[MCQ: {qid}] companyIds must be ['deloitte'], got {cids}")
    ...
    for i, q in enumerate(techs):
        ...
        if q.get("companyId") != "deloitte":
            errors.append(f"[Technical: {qid}] companyId must be 'deloitte', got '{q.get('companyId')}'")

        cids = q.get("companyIds")
        if not isinstance(cids, list) or cids != ["deloitte"]:
            errors.append(f"[Technical: {qid}] companyIds must be ['deloitte'], got {cids}")
    ...
    for i, q in enumerate(codings):
        ...
        if q.get("company") != "deloitte":
            errors.append(f"[Coding: {qid}] company must be 'deloitte', got '{q.get('company')}'")
