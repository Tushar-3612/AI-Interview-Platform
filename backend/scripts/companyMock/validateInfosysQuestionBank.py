#!/usr/bin/env python3
"""
Validation script for Infosys Company Mock Question Bank.
...
INFOSYS_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "../../data/companyMock/infosys"))

MCQ_FILE = os.path.join(INFOSYS_DIR, "mcq.json")
TECH_FILE = os.path.join(INFOSYS_DIR, "technical.json")
CODING_FILE = os.path.join(INFOSYS_DIR, "coding.json")
...
def main():
    print("=== Validating Infosys Company Mock Question Bank ===\n")
    errors = []
    warnings = []
    ...
    for i, q in enumerate(mcqs):
        ...
        if q.get("companyId") != "infosys":
            errors.append(f"[MCQ: {qid}] companyId must be 'infosys', got '{q.get('companyId')}'")

        cids = q.get("companyIds")
        if not isinstance(cids, list) or cids != ["infosys"]:
            errors.append(f"[MCQ: {qid}] companyIds must be ['infosys'], got {cids}")
    ...
    for i, q in enumerate(techs):
        ...
        if q.get("companyId") != "infosys":
            errors.append(f"[Technical: {qid}] companyId must be 'infosys', got '{q.get('companyId')}'")

        cids = q.get("companyIds")
        if not isinstance(cids, list) or cids != ["infosys"]:
            errors.append(f"[Technical: {qid}] companyIds must be ['infosys'], got {cids}")
        ...
        if topic:
            tech_topics[topic] += 1
            if topic == "System Design" or "design" in topic.lower() or "architecture" in topic.lower():
                sys_design_count += 1

    if sys_design_count < 20:
        errors.append(f"System Design question count is {sys_design_count}, expected at least 20 in Technical bank.")
    ...
    for i, q in enumerate(codings):
        ...
        if q.get("company") != "infosys":
            errors.append(f"[Coding: {qid}] company must be 'infosys', got '{q.get('company')}'")
    ...
    print(f"  System Design Questions: {sys_design_count}")
